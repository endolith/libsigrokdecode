...
regs = {
    ...
    4: [
        ...
        (
            ...
            11,
            1,
            lambda v: 'VCO Powered ' + ('Down' if v == 1 else 'Up'),
        ),
        ('Band Select Clock Divider', 12, 8, None),
        ('RF Divider Select', 20, 3, lambda v: f'÷{str(2**v)}'),
    ...
    5: [
        (
            ...
            22,
            2,
            lambda v: ['Low', 'Digital Lock Detect', 'Low', 'High'][v],
        )
    ],
}
...
class Decoder(srd.Decoder):
    ...
    def decode_bits(self, offset, width):
        return sum(
            (1 << i) if self.bits[offset + i][0] else 0 for i in range(width)
    ...
    def decode_field(self, name, offset, width, parser):
        ...
        self.put(
            ...
            pos[1],
            self.out_ann,
            [ANN_REG, [f'{name}: {parser(val) if parser else str(val)}']],
        )
    ...
    def decode(self, ss, es, data):
        ...
        if ptype == 'CS-CHANGE' and data1 == 1:
            if len(self.bits) == 32:
